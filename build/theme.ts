...
.shadow-${feature}-contrast-${depth}.${mode === "dark" ? "light" : "dark"} {
  ${
    mode === "dark"
      ? `box-shadow: 0 0 0 1px #${color}`
      : `box-shadow: inset 0 -4px ${32 / (depth * 0.5 + 1)}px ${
        buildRGBA(color, 0.1)
      }`
  }
}
...
function parsePalette(): PaletteSpecification {
  return basePalette.map(({ paletteName, swatches }) => {
    const [color, contrast] = paletteName.split("-") as [
      ColorFeature,
      "dark" | "light",
    ];
...
function buildTextColorCSS(palettes: PaletteSpecification): string {
  const blackTextColor = palettes.find(({ color, contrast }) =>
    color === "grey" && contrast === "dark"
  )?.swatches[0]?.color;
  const whiteTextColor = palettes.find(({ color, contrast }) =>
    color === "grey" && contrast === "light"
  )?.swatches[0]?.color;
  return blackTextColor && whiteTextColor
    ? makeTextColorCSS(blackTextColor, whiteTextColor)
    : "";
}
