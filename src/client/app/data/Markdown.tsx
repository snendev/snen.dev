...
import { Block, Highlight, Layer } from "../theme.tsx";
...
function MarkdownNode({ node }: MarkdownNodeProps): JSX.Element | null {
  switch (node.tag) {
    ...
    case "blockQuote": {
      const { children } = node;
      return <AsideBlock childNodes={children} />;
    }
    case "list": {
      const { children } = node;
      return (
...
function getPreviewNodes(childNodes: Node[], limitSections: number): Node[] {
  const nodes = limitSections === undefined
    ? childNodes
    : childNodes.slice(0, limitSections);
...
export default function Markdown({ slug, limitSections }: MarkdownProps) {
  ...
  const nodes = React.useMemo(
    () => limitSections ? getPreviewNodes(children, limitSections) : children,
    [children, limitSections],
  );
  return (
    <>
      {nodes.map((childNode) => <MarkdownNode node={childNode} />)}
    </>
  );
}
