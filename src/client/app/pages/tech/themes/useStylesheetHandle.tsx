...
import { Color, Grey, ColorTarget } from "../../../theme.tsx"
...
function getCSSRules(): ThemeRule[] {
  ...
  return themeStylesheet
    ? Array.from((themeStylesheet as any).cssRules).map<ThemeRule>((cssRule, index) => {
...
function initPalette(): Palette { 
  ...
  const saturations = new Array(maxDepth + 1).fill(0).map((_, i) => 94 - i * 44 / maxDepth)
  const lightnesses = new Array(maxDepth + 1).fill(0).map((_, i) => 5 + i * 33 /maxDepth)
  const contrasts = new Array(maxDepth + 1).fill(0).map((_, i) => 88 - i * 33 / maxDepth)
  return {
    hues: {
      "primary": 0,
      "accent": 0,
...
export default function useThemeStylesheetHandle(): ThemeStylesheetHandle {
  ...
  const updateSaturations = React.useCallback((depth: number, value: number) => {
    const rules = getCSSRules()
  ...
  const updateLightnesses = React.useCallback((depth: number, value: number) => {
    const rules = getCSSRules()
  ...
  const updateContrastGreys = React.useCallback((depth: number, value: number) => {
    const rules = getCSSRules()
